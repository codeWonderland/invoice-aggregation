...
pub(crate) fn parse_date(timestamp_str: &str) -> Result<NaiveDate, InternalError> {
    let datestamp = timestamp_str
        .split("T")
        .next()
        .ok_or(InternalError::DateParsing("No date found".to_string()))?;
    let date = chrono::NaiveDate::parse_from_str(datestamp, "%Y-%m-%d")
        .map_err(|e| InternalError::DateParsing(e.to_string()))?;
