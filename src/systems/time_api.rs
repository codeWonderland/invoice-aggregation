...
pub async fn get_time_entries(last_month: bool) -> Result<Vec<TimeEntry>, InternalError> {
    ...
    let month_start = NaiveDate::from_ymd_opt(now.year(), now.month(), 1)
        .ok_or_else(|| InternalError::DateParsing("Error Parsing Month Start".to_string()))?;
    ...
    start_date = NaiveDate::from_ymd_opt(start_date.year(), start_date.month(), 1)
        .ok_or_else(|| InternalError::DateParsing("Error Parsing Month Start".to_string()))?;
...
async fn get_time_entries_helper(
    start_date_str: String,
    offset: i32,
) -> Result<Vec<TimeEntry>, InternalError> {
    let api_key: String = env::var("TEAMWORK_API_KEY").expect("TEAMWORK_API_KEY must be set");

    let client = reqwest::Client::new();
    let auth_key = general_purpose::STANDARD.encode(format!("{}:f", api_key));
    let req_url = format!(
        "https://codewonderland.teamwork.com/time_entries.json?pageSize=500&page={}&fromdate={}",
        offset, start_date_str
    );
    let res = client
        .get(req_url)
        ...
        .send()
        .await
        .map_err(|e| InternalError::NetworkRequest(e.to_string()))?
        .json::<serde_json::Value>()
        .await
        .map_err(|e| InternalError::JsonParsing(e.to_string()))?;

    let parsed_entries: Vec<TimeEntry> = res
        .get("time-entries")
        .and_then(|time_entries| time_entries.as_array())
        .map(|time_entries| {
            time_entries
                .iter()
                .filter_map(|time_entry| time_entry.clone().try_into().ok())
                .collect()
        })
...
pub fn prune_entries(
    ...
    Ok(entries
        .into_iter()
        .filter(|entry: &TimeEntry| entry.date.month() == start_date.month())
        .collect())
}
