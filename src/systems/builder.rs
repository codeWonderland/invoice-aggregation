...
pub fn build_invoices(clients: Vec<Client>, last_month: bool) -> Result<(), InternalError> {
    ...
    if !exists("output").map_err(|e| InternalError::File(e.to_string()))? {
        create_dir("output").map_err(|open_err| InternalError::File(open_err.to_string()))?;
    }
    ...
    if !exists(format!("output/{}", month_string))
        .map_err(|e| InternalError::File(e.to_string()))?
    {
        create_dir(format!("output/{}", month_string))
            .map_err(|open_err| InternalError::File(open_err.to_string()))?;
    }
    ...
    for client in clients {
        ...
        writeln!(client_file, "{}", client.name).map_err(|e| InternalError::File(e.to_string()))?;
        writeln!(
            client_file,
        ...
        .map_err(|e| InternalError::File(e.to_string()))?;
        writeln!(client_file).map_err(|e| InternalError::File(e.to_string()))?;
        ...
        for list in client.entries_by_list {
            ...
            dates.sort_by_key(|a| a.day0());

            for date in dates {
                let date_entries = list
                    .entries
                    .iter()
                    .filter(|entry| entry.date == date)
                    .collect::<Vec<&TimeEntry>>();

                writeln!(client_file, "{}", date.format("%A, %B %-d"))
                    .map_err(|e| InternalError::File(e.to_string()))?;
                ...
                for entry in date_entries {
                    writeln!(
                        client_file,
                        "- {} ({} hours, {} minutes) - {}",
                        entry.task_name, entry.hours_spent, entry.minutes_spent, entry.description
                    )
                    .map_err(|e| InternalError::File(e.to_string()))?;
                }
            }
